...
public class Game {
    ...
    private Platform platformHoldingJellyfish;
     ...
     * @param timeDelta
     */
    public void updateBubbles(int secondsSinceStart, double timeDelta) {
        // Show bubbles if % 3 seconds
        if (secondsSinceStart % 3 == 0) {
            ...
            for (int i = 0; i < 3; i++) {
                ...
                for (int j = 0; j < 5; j++) {
                    ...
                    bubbles.add(new Bubble(bubbleX, height + 10));
                }
            }
        } else {
    ...
    public void testCollision(Platform other) {
        ...
        if (intersects(other) && Math.abs(jellyfish.getY() + jellyfish.getHeight() - other.getY()) < 10
            ...
            // Set the platform holding the jellyfish
            platformHoldingJellyfish = other;

            /**
             * Si la plateforme est accélérante, augmenter la vitesse de la fenêtre. 
             * J'ai diminué le multiplicateur parce que le jeu devenais trop difficle
             */
    ...
    public boolean isJellyfishOnPlatform() {return jellyfishOnPlatform;}

    public Platform getPlatformHoldingJellyfish() {return platformHoldingJellyfish;}
}
